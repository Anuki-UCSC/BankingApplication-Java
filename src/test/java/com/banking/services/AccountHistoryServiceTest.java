...
class AccountHistoryServiceTest {
    ...
    @Mock
    private AccountHistoryDataAccess accountHistoryDataAccess;
    ...
    void Should_ReturnListOfAccountHistory() {
        ...
        // when (act)
        when(accountHistoryDataAccess.getAccountHistory(accountId)).thenReturn(accountHistoryRecords);
        ArrayList<AccountHistory> actualOutput = underTest.getAccountHistory(accountId);

        // then (Assert)
        verify(accountHistoryDataAccess, Mockito.times(1)).getAccountHistory(accountId);
